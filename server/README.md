...
| Variable | Meaning |
| --- | --- |
| `INKOS_DATA_DIR` | Persistent generator jobs, package catalog, idempotency records and generated `.ink` artifacts. The container uses `/data`. |
| `INKOS_BAIDU_MAP_AK` | Optional server-only Baidu Static Map API key. Never expose it to a client or bake it into an image. |
...
The root [`docker-compose.yml`](../docker-compose.yml) runs one non-root server
with system Chromium, CJK fonts, a 512 MiB shared-memory segment and a persistent
`inkos-data` volume. Copy [`.env.example`](../.env.example) to `.env`, then use:
...
Use `docker compose up -d --build` to build the current checkout. The health
check verifies the data directory is writable, Chromium is executable, and the
dynamic time API returns the expected contract. It deliberately does not launch
Chromium on every probe.

Keep one server replica per data volume. Generator scheduling, locks and the
reused Chromium process are in-memory, and generated artifacts currently need
an operator-defined retention policy.
