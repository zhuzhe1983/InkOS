import { getInkCatalogPackage, getInkFrameArtifact } from "@/lib/ink/catalog-store";
...
export async function GET(request: Request, context: RouteContext): Promise<Response> {
  try {
    ...
    const artifact = getInkFrameArtifact(loaded, variantId, uuid, pageIndex);
    if (!artifact) {
      return catalogNotFound(request, "FRAME_NOT_FOUND", "Frame not found", "The package does not contain that exact frame");
    }
